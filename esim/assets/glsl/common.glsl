...
//==================================================================
void ONAS_CalcRayFromCameraLen(vec3 pos, out vec3 out_raySta, out vec3 out_rayDir, out float out_rayLen) {
  out_raySta = u_CameraPos;
...
//==================================================================
vec3 ONAS_RaytraceScatterSky(vec3 raySta, vec3 rayDir, float rayLen, float useOuterRadius, float near, float startDepth) {
...
//==================================================================
void ONAS_RaytraceScatterGround(out vec3 out_groundCol, out vec3 out_atten, vec3 pos, vec3 raySta, vec3 rayDir, float rayLen, float useOuterRadius, float near) {
    // Calculate the ray's starting position,
    //   then calculate its scattering offset
...
//==================================================================
void ONAS_CalcMieAndRayleighForSky(out vec3 out_mieCol, out vec3 out_rayleighCol, vec3 raySta, vec3 rayDir, float rayLen, float useOuterRadius, float near, float startDepth) {
  vec3 baseCol = ONAS_RaytraceScatterSky(raySta, rayDir, rayLen, useOuterRadius, near, startDepth);
...
void ONAS_CalcMieAndRayleighForSkyInside(out vec3 out_mieCol, out vec3 out_rayleighCol, out vec3 out_posToCam, vec3 pos) {
  ...
  float startDepth = exp(u_ScaleOverScaleDepth *
    (u_InnerRadius - useOuterRadius));

  ONAS_CalcMieAndRayleighForSky(out_mieCol, out_rayleighCol, raySta, rayDir, rayLen, useOuterRadius, near, startDepth);
...
void ONAS_CalcMieAndRayleighForSkyOutside(out vec3 out_mieCol, out vec3 out_rayleighCol, out vec3 out_posToCam, vec3 pos) {
  ...
  float near = ONAS_CalcRaySphereClosestInters(raySta, rayDir, vec3(0.0, 0.0, 0.0), useOuterRadius * useOuterRadius);
...
void ONAS_CalcColorsForGroundInside(out vec3 out_groundCol, out vec3 out_attenuation, vec3 pos) {
  ...
  ONAS_RaytraceScatterGround(out_groundCol, out_attenuation, pos, raySta, rayDir, rayLen, ONAS_CalcCamDistanceFromPlanetOrigin(), 0.0);
}
