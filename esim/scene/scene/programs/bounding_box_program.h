#ifndef __ESIM_SCENE_SCENE_PROGRAMS_BOUNDING_BOX_PROGRAM_H_
#define __ESIM_SCENE_SCENE_PROGRAMS_BOUNDING_BOX_PROGRAM_H_
...
/**
 * @brief The vertex details of bounding-box shader
 * 
 */
...
class bounding_box_program final : public gl::program {
public:
  /**
  ...
  inline void enable_position_pointer() const noexcept {
    ...
    glVertexAttribPointer(a_pos_, 3,
                          GL_FLOAT, GL_FALSE,
                          sizeof(bounding_box_vertex), (void *)0);
  }
   ...
   */
  bounding_box_program(gl_error_callback err_cb = nullptr) noexcept
      : gl::program{err_cb},
    ...
    assert(read_file("glsl/bounding_box.vert", vs_text));
    assert(read_file("glsl/bounding_box.frag", fs_text));
   ...
   */
  inline static r_ptr<bounding_box_program> get() noexcept {
    static u_ptr<bounding_box_program> shader__;

    if (nullptr != shader__) {

      return shader__.get();
    } else {
      shader__ = make_ptr_u<bounding_box_program>();
