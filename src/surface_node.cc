...
public:
  ...
  void prepare_vertex() noexcept {
    ...
    for (size_t i=0; i<=details; ++i) {
      for (size_t j=0; j<=details; ++j) {
        ...
        auto &vb = v_buffer[to_index(i, j)];
        vb.lat = float(buffer.x = coord::tiley_to_lati(zoom, ytile + tile_step * i));
        vb.lon = float(buffer.y = coord::tilex_to_long(zoom, xtile + tile_step * j));
        buffer.z = 0.0;
  ...
  void draw(const camera &cmr) noexcept {
    ...
    glUniformMatrix4fv(unf_vp_, 1, GL_FALSE, glm::value_ptr(cmr.get_vp()));
    ...
    if (curr != x) {
      curr = x;
      printf("%d => LLA(%lf, %lf), ECEF (%lf, %lf, %lf)\n", curr, vbo_.at(ibo_.at(curr)).lat,
                                                                  vbo_.at(ibo_.at(curr)).lon,
                                                                  vbo_.at(ibo_.at(curr)).x,
                                                                  vbo_.at(ibo_.at(curr)).y,
                                                                  vbo_.at(ibo_.at(curr)).z);
      printf("%d => LLA(%lf, %lf), ECEF (%lf, %lf, %lf)\n", curr+1, vbo_.at(ibo_.at(curr+1)).lat,
                                                                    vbo_.at(ibo_.at(curr+1)).lon,
                                                                    vbo_.at(ibo_.at(curr+1)).x,
                                                                    vbo_.at(ibo_.at(curr+1)).y,
                                                                    vbo_.at(ibo_.at(curr+1)).z);
      printf("%d => LLA(%lf, %lf), ECEF (%lf, %lf, %lf)\n\n", curr+2, vbo_.at(ibo_.at(curr+2)).lat,
                                                                      vbo_.at(ibo_.at(curr+2)).lon,
                                                                      vbo_.at(ibo_.at(curr+2)).x,
                                                                      vbo_.at(ibo_.at(curr+2)).y,
                                                                      vbo_.at(ibo_.at(curr+2)).z);
    }
...
private:
  static const uint32_t details = 15;
